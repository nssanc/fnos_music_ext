...
- **播放音源可见且可切换**：搜索结果与播放页保留歌曲来源信息，点击播放页的「音源」入口会显示 QQ、网易云、酷我/咪咕和已启用的洛雪音乐源并可手动切换；洛雪也可设为优先歌曲源。兼容 `audio.src`、Fetch/XHR、HLS 以及被飞牛重新扫描为本地 GUID 的在线缓存歌曲。无法识别当前歌曲时会显示原因和设置入口，不再静默无反应。自动模式会综合标题、歌手、时长及完整流验证选择来源，手动选择仍会阻止明显的 30 秒试听地址。
