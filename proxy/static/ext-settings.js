(() => {
  ...
  const sourceNames = { musicdl: '酷我 / 咪咕', netease: '网易云音乐', qqmusic: 'QQ 音乐', lx: '洛雪自定义源', lxmusic: '洛雪聚合搜索' };
  let qrTimer = null;
  ...
  async function load(root) {
    ...
    try {
      ...
      body.innerHTML = `<div class="fmx-section"><h3>歌曲与歌词来源</h3><div class="fmx-card"><label class="fmx-name">优先歌曲源</label><select class="fmx-input" data-preference="audioSource"><option value="auto" ${selected(prefs.audioSource, 'auto')}>自动选择（推荐）</option><option value="qqmusic" ${selected(prefs.audioSource, 'qqmusic')}>QQ 音乐优先</option><option value="netease" ${selected(prefs.audioSource, 'netease')}>网易云优先</option><option value="musicdl" ${selected(prefs.audioSource, 'musicdl')}>酷我 / 咪咕优先</option><option value="lxmusic" ${selected(prefs.audioSource, 'lxmusic')}>洛雪聚合优先</option><option value="lx" ${selected(prefs.audioSource, 'lx')}>洛雪自定义源优先</option></select><div class="fmx-muted" style="margin-top:7px">同名歌曲会保留不同来源，标题后的〔来源〕可直接区分和切换。</div><label class="fmx-name" style="display:block;margin-top:14px">歌词源</label><select class="fmx-input" data-preference="lyricSource"><option value="auto" ${selected(prefs.lyricSource, 'auto')}>自动选择（推荐）</option><option value="same" ${selected(prefs.lyricSource, 'same')}>跟随歌曲源</option><option value="qqmusic" ${selected(prefs.lyricSource, 'qqmusic')}>QQ 音乐歌词</option><option value="netease" ${selected(prefs.lyricSource, 'netease')}>网易云歌词</option><option value="musicdl" ${selected(prefs.lyricSource, 'musicdl')}>酷我 / 咪咕歌词</option><option value="lxmusic" ${selected(prefs.lyricSource, 'lxmusic')}>洛雪聚合歌词</option><option value="lx" ${selected(prefs.lyricSource, 'lx')}>洛雪自定义歌词</option></select></div></div><div class="fmx-section"><h3>内置音源</h3>${data.builtins.map(s => `
  ...
  async function openSourceMenu() {
    ...
    try {
      ...
      const labels={qqmusic:'QQ 音乐',netease:'网易云音乐',musicdl:'酷我 / 咪咕',lxmusic:'洛雪聚合',lx:'洛雪自定义源'};
  ...
  function updatePlayerSource() {
    ...
    if(!label) { const src=track.guid.startsWith('online:')?track.guid.split(':')[1]:''; label={qq:'QQ 音乐',netease:'网易云',kuwo:'酷我',migu:'咪咕',kugou:'酷狗',lx:'洛雪聚合',lxsource:'洛雪自定义源'}[src] || (src?'在线音源':'本地音乐'); }
